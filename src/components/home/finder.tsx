            ...
            <div className="filter-col" data-grid="2">
              ...
              <div className="chips">
                <button
                  type="button"
                  className={`chip ${
                    p.prices.size === 0 && !p.priceRange ? "on" : ""
                  }`}
                  onClick={() => {
                    p.setPrices(new Set());
                    p.setPriceRange(null);
                  }}
                >
                  Összes
                </button>
                {p.bands.map((b) => (
                  <button
                    ...
                    onClick={() => {
                      p.setPriceRange(null);
                      p.setPrices(toggleSet(p.prices, b.id));
                    }}
                  >
                    {b.label_hu}
                  </button>
...
function PriceSlider({
  ...
  const dragStart = useRef<{ x: number; lo: number; hi: number }>({
    x: 0,
    lo: 0,
    hi: 0,
  });
  ...
  function valueFromX(clientX: number) {
    ...
    if (!w) return absMin;
    const ratio = (clientX - w.left) / w.width;
    return Math.round((absMin + ratio * span) * 10) / 10;
  }
  ...
  function onMove(e: React.PointerEvent) {
    ...
    if (drag.current === "lo") {
      const next = clamp(
        Math.round((dragStart.current.lo + dx) * 10) / 10,
        absMin,
        hi - MIN_GAP,
      );
      setLo(next);
      ...
      const next = clamp(
        Math.round((dragStart.current.hi + dx) * 10) / 10,
        lo + MIN_GAP,
        absMax,
      );
      setHi(next);
      ...
      let nextLo = Math.round((dragStart.current.lo + dx) * 10) / 10;
      nextLo = clamp(nextLo, absMin, absMax - width);
  ...
  function handleTrackClick(e: React.PointerEvent) {
    ...
    if (Math.abs(v - lo) < Math.abs(v - hi)) {
      const next = clamp(v, absMin, hi - MIN_GAP);
      setLo(next);
      commit(next, hi);
    } else {
      const next = clamp(v, lo + MIN_GAP, absMax);
      setHi(next);
      commit(lo, next);
    }
  }
  ...
  return (
    <div
      className={`price-slider ${active ? "active" : ""}`}
      aria-label="Ársáv finomhangolása"
    >
      <div className="ps-readout">
        <span className="ps-mode">
          {active
            ? "Egyedi ársáv · csúszka"
            : anyChip
              ? "Sávok kiválasztva"
              : "Teljes ársáv"}
        </span>
      ...
      >
        ...
        />
        <div
          className="ps-handle ps-handle--lo"
          tabIndex={0}
          role="slider"
        ...
        />
        <div
          className="ps-handle ps-handle--hi"
          tabIndex={0}
          role="slider"
      ...
      <div className="ps-ends">
        ...
        <button
          type="button"
          className={`ps-reset ${active ? "" : "hidden"}`}
          onClick={reset}
        >
          Visszaállítás
        </button>
