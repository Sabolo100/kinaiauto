...
| Object storage | **MinIO on Coolify** (one-click template, ~200 MB RAM). Note the **API** URL (S3 traffic) vs the console URL — use the API URL everywhere. Buckets + public-read are created by `00-minio-buckets.mts`. |
